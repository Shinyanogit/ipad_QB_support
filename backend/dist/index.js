...
app.post("/chat", async (req, res) => {
    ...
    const payload = { model: RESTRICTED_MODEL, messages };
...
function initializeFirebase() {
    ...
    (0, app_1.initializeApp)({ projectId: FIREBASE_PROJECT_ID });
}
...
async function enforceUsagePolicy(uid, requestedModel) {
...
async function consumeRateLimit(uid) {
    ...
    return db.runTransaction(async (tx) => {
        ...
        tx.set(ref, { windowStart, count: nextCount, updatedAt: now }, { merge: true });
        return {
            allowed: true,
...
function isAllowedUser(decoded) {
    if (!ALLOWED_EMAILS.length && !ALLOWED_DOMAIN)
        return true;
